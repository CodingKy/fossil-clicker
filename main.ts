...
function start_screen () {
    if (!(DEBUG)) {
        ...
        text_sprite_temp = textsprite.create("Fossil", 0, 1)
        text_sprite_temp.setMaxFontHeight(16)
        text_sprite_temp.top = 11
        text_sprite_temp.x = 80
        text_sprite_temp = textsprite.create("Clicker", 0, 1)
        text_sprite_temp.setMaxFontHeight(16)
        text_sprite_temp.top = 27
        text_sprite_temp.x = 80
        text_sprite_temp = textsprite.create("A game by KM Studios", 0, 1)
        text_sprite_temp.top = 45
        text_sprite_temp.x = 80
        text_sprite_temp = textsprite.create("", 0, 15)
...
function show_upgrades_menu (transition: boolean) {
    ...
    for (let value of upgrades) {
        // Upgrade format:
        // title | description | $price | effect*=xdads
    ...
    if (game_state == "game") {
        if (cursor_enabled) {
            if (sprite_cursor.overlapsWith(sprite_main_icon)) {
                ...
                for (let value2 of sprites_towers) {
                    if (sprite_cursor.overlapsWith(value2)) {
                        show_tower_menu([value2], true)
                    }
                }
            }
        }
...
function short_scale_divider (num: number) {
    for (let index2 = 0; index2 <= short_scale_names.length; index2++) {
        if (num < 10 ** ((index2 + 1) * 3) || index2 == short_scale_names.length - 1) {
            return 10 ** (index2 * 3)
        }
    }
    return 0
...
function save_game () {
    ...
    for (let value3 of sprites_towers) {
        blockSettings.writeNumber("count_" + sprites.readDataString(value3, "internal_name"), sprites.readDataNumber(value3, "count"))
    }
...
function calculate_sell_price (tower_in_list: Sprite[], count: number) {
    ...
    for (let index5 = 0; index5 <= count - 1; index5++) {
        local_sum += primitive_tower_price(sprites.readDataNumber(local_sprite, "price"), sprites.readDataNumber(local_sprite, "count") - 1 - index5)
    }
    return local_sum
}
...
function load_game () {
    ...
    if (blockSettings.readBoolean("has_game_save")) {
        ...
        for (let value4 of sprites_towers) {
            sprites.setDataNumber(value4, "count", blockSettings.readNumber("count_" + sprites.readDataString(value4, "internal_name")))
        }
...
function short_scale_name (num: number) {
    for (let index6 = 0; index6 <= short_scale_names.length; index6++) {
        if (num < 10 ** ((index6 + 1) * 3) || index6 == short_scale_names.length - 1) {
            return short_scale_names[index6]
        }
    }
    return 1
}
function recalculate_fossils_per_sec () {
    ...
    for (let value5 of sprites_towers) {
        sprites.setDataNumber(value5, "speed_multiplier", 1)
    }
    for (let value6 of upgrades_purchased) {
        local_upgrade = value6.split(" | ")
        ...
        if (local_target == "price") {
            ...
            for (let value7 of sprites_towers) {
                if (local_target == sprites.readDataString(value7, "internal_name")) {
                    sprites.setDataNumber(value7, "speed_multiplier", sprites.readDataNumber(value7, "speed_multiplier") * local_effect_value)
                    break;
                }
            }
        }
    }
    for (let value8 of sprites_towers) {
        fossils_per_second += sprites.readDataNumber(value8, "count") * sprites.readDataNumber(value8, "speed") * sprites.readDataNumber(value8, "speed_multiplier")
    }
}
...
function calculate_buy_price (tower_in_list: Sprite[], count: number) {
    local_sprite = tower_in_list[0]
    local_sum = 0
    for (let index7 = 0; index7 <= count - 1; index7++) {
        local_sum += primitive_tower_price(sprites.readDataNumber(local_sprite, "price"), sprites.readDataNumber(local_sprite, "count") + index7)
    }
    return local_sum
}
...
function show_game_menu (transition: boolean) {
    for (let value9 of sprites.allOfKind(SpriteKind.MiniMenu)) {
        slide_out_menu(value9)
    }
    enable_cursor(false)
...
game.onUpdate(function () {
    if (game_state == "game") {
        ...
        update_top_bar_text()
        for (let value10 of sprites_towers) {
            update_tower_button([value10])
        }
